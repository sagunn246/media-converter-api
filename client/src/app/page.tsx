...
export default function Home() {
  ...
  const saveToHistory = (item: any) => {
    ...
    const updated = [newItem, ...history.filter((h) => h.filename !== newItem.filename)].slice(0, 10);
    setHistory(updated);
    try {
      localStorage.setItem("audiopulse_history", JSON.stringify(updated));
    } catch {}
  };

  const clearHistory = () => {
    setHistory([]);
    try { localStorage.removeItem("audiopulse_history"); } catch {}
  };
